...
class HydraulicSystemFull(System):
    ...
    def __init__(
        ...
        *args,
        system_parameters_init = {
            ...
            "x_th_eps": 1.0, # backlash (should be as small as possible)
    ...
    def get_pressure_hydraulic(self, state) -> float:
        ...
        # # Calculate
        # if x_th > x_th_eps: # NOTE: WAS x_th > 0
        #     pressure_hydraulic = p_l
        #     # dynamic pressure loss happends only when there is a flow rate
        #     if v_p != 0: 
        #         # self.x_th_eps refers to somekind of backslash
        #         pressure_hydraulic -= v_p*(abs(v_p)/\
        #             x_th**2)*ploss_coef_hydr # NOTE: WAS max(x_th_eps, x_th)**2)*ploss_coef_hydr
    ...
    def get_force_friction(self, state, F_h: float) -> float:
        ...
        Args:
            state:
            state (array): array of current state:
                x_p (float): piston position [µm]. Do not required
                v_p (float): piston velocity [µm/s]
                x_th (float): throttle position [µm]. Do not required
    ...
    def _compute_state_dynamics(self, time, state, inputs):
        ...
        x_th = np.clip(state[2], x_th_limits[0], x_th_limits[1])
...
class HydraulicSystemSimpleClose(HydraulicSystemFull):
    ...
    def get_pressure_hydraulic(self, state) -> float:
        ...
        if x_th > x_th_eps: # NOTE: WAS x_th > 0
            ...
            if v_p != 0: 
                # self.x_th_eps refers to somekind of backslash
                pressure_hydraulic -= v_p*(abs(v_p)/max(x_th_eps, x_th)**2)*ploss_coef_hydr # NOTE: WAS max(x_th_eps, x_th)**2)*ploss_coef_hydr
            ...
            # pressure_hydraulic = p_hydr_last + (x_p_last/x_p - 1)/beta_v_hydr
            pressure_hydraulic = self._parameters["p_hydr_init"]
