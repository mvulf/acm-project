...
class HydraylicAnimation():
    def __init__(
        ...
        self.plot_dict = {3: {'data': self.l_jet_vec_init,
                              'color': 'red',
                              'y_tit': r'$x_\mathrm{стр}$, мм',
                              'label1': r'$\hat{x}_\mathrm{стр}$',
                              'label2': r'$x_\mathrm{стр}$',
                              'legend_loc': 'center right',
                              'observation': self.obs_jet_length,
                              'h_line': l_critic},
                    0: {'data': self.p_down_vec_init, 'data2': self.p_up_vec_init,
                         'color': 'red', 'color2': 'teal',
                         'label1': r'$p_\mathrm{рабоч}$',
                         'label2': r'$p_\mathrm{гидр}$',
                         'legend_loc': 'center right',
                           'y_tit': r'$p$, кПа'},
                    2: {'data': self.x_th_vec_init,
                        'data2': None,
                        'actions': True,
                        'label1': r'$x_\mathrm{др}$',
                        'label2': r'$x^\mathrm{уст}_\mathrm{др}$',
                        'legend_loc': 'upper right',
                        'color': 'green', 'color2': 'red',
                        'h_line': 0,
                        'y_tit': r'$x_\mathrm{др}$, мкм'},
                    4: {'data': self.v_jet_vec_init,
                         'color': 'blue',
                         'y_tit': r'$v_\mathrm{стр}$, мм/c',
                         'label1': r'$\hat{v}_\mathrm{стр}$',
                         'label2': r'$v_\mathrm{стр}$',
                         'legend_loc': 'upper right',
    ...
    def init_plot(self):
        ...
        self.axes[0].set_position([0.10, 0.10, 0.3, 0.38])
        self.axes[1].set_position([0.41, 0.05, 0.2, 0.9])
        self.axes[2].set_position([0.10, 0.60, 0.3, 0.38])
        self.axes[3].set_position([0.65, 0.10, 0.3, 0.38])
        self.axes[4].set_position([0.65, 0.60, 0.3, 0.38])
    ...
    def animate(self, i):
        ...
        for j, ax in enumerate(self.axes):
            ...
            if j != 1: # the 1th axis is used form plotting system
                ...
                self.axes[j].set_xlabel(xlabel=r'$\tau$, мс', fontsize=10)
    ...
    def animate_system(self, interacitve=False, save_gif = False, gif_filename = 'pistons.gif'):
        ...
        if save_gif:
            ...
            writer = animation.PillowWriter(fps=80, # was 40
                                            codec="gif",
                                            metadata=dict(artist='Me'),
                                            bitrate=5000)
            
            anim.save(gif_filename, writer=writer, dpi=200, savefig_kwargs={"transparent": False, 'facecolor':'red'}) # was dpi=300
