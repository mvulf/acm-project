...
class HydraulicSystem(System):
    ...
    def get_pressure_hydraulic(self, state) -> float:
        ...
        # pressure_hydraulic_1 = rg.if_else(x_th_eps > x_th, x_th_eps, x_th)
        # rg.if_else(v_p < 0, -1 * v_p, v_p)
    ...
    def get_pressure_working(self, state) -> float:
        ...
        pressure_capillar = rg.if_else(
            p_capillar_max < p_compress, p_capillar_max, p_compress
        )
        
        # pressure_working = p_atm + np.sign(dx_p) * pressure_capillar
        pressure_working = p_atm + rg.sign(dx_p) * pressure_capillar
            
        # dynamic pressure loss happends only when there is a flow rate
        # if v_p != 0:
        #     pressure_working += v_p*rg.if_else(v_p<0, -1*v_p, v_p) * ploss_coef_work #!!!!!!!!
    ...
    def get_acceleration(self, state) -> float:
        ...
        # cond_1 = rg.if_else(v_p != 0, 1, 0)
        # cond_2_1 = rg.abs(F_h + F_g) 
        # cond_2_2 = rg.abs(F_fr)
        # cond_2 = rg.if_else(cond_2_1 > cond_2_2, 1, 0)

        # to_return = rg.if_else(cond_1+cond_2 > 1, (g + 1/m_p * (F_h + F_fr))*1e6, 0)
    ...
    def _compute_state_dynamics(self, time, state, inputs):
        ...
        # cond = rg.if_else(rg.abs(x_th_act - x_th) > dx_th_eps, 1, 0)
